...
const VideoElement = ({ stream }: VideoElementProps) => {
  ...
  useEffect(() => {
    ...
    if (stream) {
      ...
      const checkVolume = () => {
        ...
        const volume = dataArray.reduce((sum, value) => sum = sum + value, 0) / dataArray.length
  ...
  return <div className='flex w-72 h-40 justify-center items-center hover:cursor-pointer rounded-lg relative'>
    ...
    <div
      ...
      <video ref={videoRef} autoPlay muted className={`rounded-lg bg-black w-full h-full object-cover`}></video>
